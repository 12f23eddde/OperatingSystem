...
FileSystem::FileSystem(bool format) {
    ...
    if (format) {
        ...
        mapHdr->fileType = bitmapFile;
        mapHdr->timeCreated = stats->totalTicks;
        dirHdr->fileType = dirFile;
        dirHdr->timeCreated = stats->totalTicks;
...
bool
FileSystem::Create(char *name, int initialSize, FileType fileType) {
    Directory *directory;
    BitMap *freeMap;
    FileHeader *hdr;
    int sector;
...
void HeaderTable::printTable(){
    ...
    for(int i = 0; i < tableSize; i++){
        if(table[i].inUse){
            printf("%5d %5d\n", table[i].hdrSector, table[i].readerCount);
        }
    }
}
#endif